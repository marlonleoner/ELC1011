...
main:
#
#   Programa para testar os procedimentos leia_linha e imprime_string. Estes
#   procedimentos são usados com a ferramenta keyboard and display MMIO Simulator.
#   O procedimento leia_linha faz a leitura de uma linha da ferramenta keyboard
#   e armazena temporariamente em uma string (buffer_linha).
#   O procedimento imprime_string envia uma string para a ferramenta display.
#   Este programa lê uma linha do keyboard e envia para o display.
#
################################################################################
laco_infinito:
            la      $a0, buffer_linha
            ...
            li      $t1, '\0'
            sb      $t1, 0($t0)
            # imprimimos na ferramenta display
            la      $a0, buffer_linha
            jal     imprime_string
            j laco_infinito
#-------------------------------------------------------------------------------
